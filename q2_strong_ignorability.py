#!/usr/bin/env python3
"""Q2 analysis under strong ignorability (exogenous switching).

This script answers Q2 using point estimates (difference in means) as if treatment assignment is unconfounded.
"""
...
def analyze_q2_strong_ignorability(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for group_name, gdf in {
        ...
        for outcome in ["Prom__20162", "Tasa_aprob_20162"]:
            treated = gdf[gdf["Continue2"] == 1][outcome].dropna()
            control = gdf[gdf["Continue2"] == 0][outcome].dropna()
...
def write_report(results: pd.DataFrame, output_dir: str) -> str:
    report_path = os.path.join(output_dir, "q2_strong_ignorability_report.md")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Q2 strong ignorability analysis")
    parser.add_argument("--data-path", default="Data2026.xls", help="Input dataset path")
    parser.add_argument("--output-dir", default="outputs_q2_strong_ignorability", help="Output directory")
    return parser
